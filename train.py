...
TRAINING_STEPS = [8000, 4000, 4000, 4000]
...
VALID_LOSS_STREAK_MAX = [10, 10, 10, 10, 10, 10, 10, 9, 9, 8, 8, 8, 7, 7, 7, 7, 7, 6, 6, 6, 6, 5, 5, 5, 5]
    ...
    for training_step in range(start_step, max_training_steps + 1):
        ...
        if (training_step % VAL_INTERVAL) == 0 or (training_step == max_training_steps):
            ...
            for val_step in range(0, val_size, FLAGS.batch_size):
                val_x, val_y_seg, val_y_con = data_processor.batch(FLAGS.batch_size, offset=val_step, mode='valid', invert=FLAGS.invert)
            ...
            if valid_loss < best_valid_loss:
                ...
                valid_loss_streak += 1
                if valid_loss_streak >= VALID_LOSS_STREAK_MAX[valid_loss_streak_hits]:
                    valid_loss_streak = 0
                    valid_loss_streak_hits += 1
                    tf.logging.info(f"Valdation loss has not improved for {VALID_LOSS_STREAK_MAX[valid_loss_streak_hits]} steps")
